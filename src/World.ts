...
export abstract class World {
  ...
  public static createCamera(position: Vector3, rotation: Vector3, fieldOfView: number): Camera {
    return new Camera(
      CreateCamWithParams('DEFAULT_SCRIPTED_CAMERA', position.x, position.y, position.z, rotation.x, rotation.y, rotation.z, fieldOfView, true, 2),
    );
  }
   ...
   * @returns Ped object.
   */
  public static async createPed(model: Model, position: Vector3, heading = 0): Promise<Ped | undefined> {
   ...
   */
  public static async createVehicle(model: Model, position: Vector3, heading = 0): Promise<Vehicle | undefined> {
   ...
   * @returns Vehicle object.
   */
  public static async createRandomVehicle(position: Vector3, heading = 0): Promise<Vehicle | undefined> {
  ...
  public static async createProp(model: Model, position: Vector3, dynamic: boolean, placeOnGround: boolean): Promise<Prop | undefined> {
    if (!model.IsProp || !(await model.request(1000))) {
      return undefined;
    }

    const prop = new Prop(CreateObject(model.Hash, position.x, position.y, position.z, true, true, dynamic));
  ...
  public static async createPickup(pickupHash: PickupHash, position: Vector3, value = 0, model: Model): Promise<Pickup | undefined> {
    if (!pickupHash) {
      return undefined;
    }

    if (!model.IsValid || !(await model.request(1000))) {
      return undefined;
    }

    return new Pickup(CreatePickup(pickupHash, position.x, position.y, position.z, 0, value, false, model.Hash));
  }
   ...
   */
  public static raycast(source: Vector3, direction: Vector3, maxDistance: number, options: IntersectOptions, ignoreEntity: Entity): RaycastResult {
