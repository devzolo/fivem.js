...
export class VehicleModCollection {
  ...
  public getMod(modType: VehicleModType): VehicleMod | undefined {
    if (!this._vehicleMods.has(modType)) {
      this._vehicleMods.set(modType, new VehicleMod(this.owner, modType));
    }
  ...
  public getToggleMod(modType: VehicleToggleModType): VehicleToggleMod | undefined {
    if (!this._vehicleToggleMods.has(modType)) {
      this._vehicleToggleMods.set(modType, new VehicleToggleMod(this.owner, modType));
    }
    return this._vehicleToggleMods.get(modType);
  }

  public getAllMods(): (VehicleMod | undefined)[] {
  ...
  public get Livery(): number {
    const modCount = this.getMod(VehicleModType.Livery)?.ModCount ?? 0;
    if (modCount > 0) {
      return this.getMod(VehicleModType.Livery)?.Index ?? -1;
    }
    return GetVehicleLivery(this.owner.Handle);
  }

  public set Livery(value: number) {
    let mod = this.getMod(VehicleModType.Livery);
