...
export class Menu {
  ...
  private counterOverride: string | undefined = undefined;
  ...
  private readonly subtitleResText: ResText | undefined;
  private readonly subtitleResRectangle: ResRectangle | undefined;
  ...
  private readonly counterText: ResText | undefined;
  private readonly background: Sprite;

  constructor(title: string, subtitle: string, offset = new Point(15, 15), spriteLibrary?: string, spriteName?: string) {
    ...
    this.logo = new Sprite(this.spriteLibrary, this.spriteName, new Point(0 + this.offset.X, 0 + this.offset.Y), new Size(431, 107));
    ...
    if (this.subtitle !== '') {
      this.mainMenu.addItem(
        (this.subtitleResRectangle = new ResRectangle(new Point(0 + this.offset.X, 107 + this.offset.Y), new Size(431, 37), new Color(255, 0, 0, 0))),
      );
      this.mainMenu.addItem(
    ...
    this.descriptionRectangle = new Sprite('commonmenu', 'gradient_bgd', new Point(this.offset.X, 127), new Size(431, 30));
  ...
  public set Subtitle(text: string) {
    this.subtitle = text;
    if (this.subtitleResText) this.subtitleResText.caption = text;
  }
  ...
  public set SubtitleForeColor(color: Color) {
    if (this.subtitleResText) this.subtitleResText.color = color;
  }

  public get SubtitleForeColor(): Color {
    return this.subtitleResText?.color ?? new Color(255, 255, 255, 255);
  }

  public set SubtitleBackColor(color: Color) {
    if (this.subtitleResRectangle) this.subtitleResRectangle.color = color;
  }

  public get SubtitleBackColor(): Color {
    return this.subtitleResRectangle?.color ?? new Color(255, 255, 255, 255);
  }

  public goLeft(): void {
    if (!(this.menuItems[this.CurrentSelection] instanceof UIMenuListItem) && !(this.menuItems[this.CurrentSelection] instanceof UIMenuSliderItem)) {
      return;
    }
