...
export class UIMenuItem {
  ...
  private event: { event: string; args: unknown[] } | undefined;
  ...
  public draw(): void {
    this.rectangle.size = new Size(431 + (this.parent?.widthOffset ?? 0), 38);
    this.selectedSprite.size = new Size(431 + (this.parent?.widthOffset ?? 0), 38);
    ...
    this.text.color = this.enabled ? (this.selected ? this.highlightedForeColor : this.foreColor) : new Color(255, 163, 159, 148);
    ...
    if (this.rightBadge !== BadgeStyle.None) {
      this.badgeRight.pos = new Point(385 + this.offset.X + (this.parent?.widthOffset ?? 0), this.badgeRight.pos.Y);
    ...
    if (this.rightLabel && this.rightLabel !== '') {
      this.labelText.pos = new Point(420 + this.offset.X + (this.parent?.widthOffset ?? 0), this.labelText.pos.Y);
