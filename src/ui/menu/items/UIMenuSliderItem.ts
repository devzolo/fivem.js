...
export class UIMenuSliderItem extends UIMenuItem {
  ...
  constructor(text: string, items: unknown[], index: number, description = '', divider = false, arrowOnlyOnSelected = false) {
  ...
  public setVerticalPosition(y: number): void {
    this.rectangleBackground.pos = new Point(250 + this.offset.X + (this.parent?.widthOffset ?? 0), y + 158.5 + this.offset.Y);
    this.rectangleSlider.pos = new Point(250 + this.offset.X + (this.parent?.widthOffset ?? 0), y + 158.5 + this.offset.Y);
    this.rectangleDivider.pos = new Point(323.5 + this.offset.X + (this.parent?.widthOffset ?? 0), y + 153 + this.offset.Y);
    this.arrowLeft.pos = new Point(235 + this.offset.X + (this.parent?.widthOffset ?? 0), 155.5 + y + this.offset.Y);
    this.arrowRight.pos = new Point(400 + this.offset.X + (this.parent?.widthOffset ?? 0), 155.5 + y + this.offset.Y);
  ...
  public draw(): void {
    super.draw();
    this.arrowLeft.color = this.enabled ? (this.selected ? Color.black : Color.whiteSmoke) : new Color(255, 163, 159, 148);
    this.arrowRight.color = this.enabled ? (this.selected ? Color.black : Color.whiteSmoke) : new Color(255, 163, 159, 148);
    const offset = ((this.rectangleBackground.size.width - this.rectangleSlider.size.width) / (this.items.length - 1)) * this.Index;
    this.rectangleSlider.pos = new Point(250 + this.offset.X + offset + +(this.parent?.widthOffset ?? 0), this.rectangleSlider.pos.Y);
